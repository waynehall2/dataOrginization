...
def add_comment_based_on_value(sheet_obj, value, comment_text, author_name):
    """
    Adds a comment to cells containing a specific value in a given sheet.

    Parameters:
    - sheet_obj: The sheet object to search through.
    - value: The value to match in cells.
    - comment_text: The text to add as a comment.
    - author_name: The author of the comment.
    """
    for row in sheet_obj.iter_rows():
        for cell in row:
            if cell.value == value:
                comment = Comment(comment_text, author=author_name)
                cell.comment = comment

# Add comments to skill levels
for sheet_name in wb.sheetnames:
    sheet_obj = wb[sheet_name]

    add_comment_based_on_value(sheet_obj, "Beginner", "I understand the basics.", "Author Name")
    add_comment_based_on_value(sheet_obj, "Intermediate", "I apply this skill in my work and continue to learn more.", "Author Name")
    add_comment_based_on_value(sheet_obj, "Advanced", "I have applied the skill in multiple contexts and can educate others.", "Author Name")
